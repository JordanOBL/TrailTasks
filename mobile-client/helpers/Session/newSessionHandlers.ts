...
NewSessionHandlers.checkLocalStorageSessionSettings = async ({
  ...
  const recentSettings: string | void = await database.localStorage.get(
    "category " + sessionCategoryId + " settings",
  );

  return recentSettings ? JSON.parse(recentSettings) : {};
};
...
NewSessionHandlers.SelectSessionCategoryId = async ({
  ...
  try {
    const recentSettings = await NewSessionHandlers.checkLocalStorageSessionSettings({
      sessionCategoryId,
      database,
    });
    if (recentSettings && Object.values(recentSettings).length > 1) {
      setTimer((prev: any) => ({
        ...prev,
        focusTime: recentSettings.focusTime,
        shortBreakTime: recentSettings.shortBreakTime,
        longBreakTime: recentSettings.longBreakTime,
        time: recentSettings.focusTime,
      }));
    }
...
NewSessionHandlers.FocusTimeChange = ({
  setTimer,
  value,
}: {
  setTimer: React.Dispatch<React.SetStateAction<Timer>>;
