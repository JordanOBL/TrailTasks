import {Addon, Trail, User, User_Addon, User_Park, User_Purchased_Trail} from '../models';
...
describe('users writers', () => {
  describe('purchaseTrail()',  () => {
    ...
    test('succesfully purchases and adds trail to userspurchased trails', async () => {
      ...
      const [ user  ]: User[] = await testDb.get<User>('users').query( Q.where('email', 'mockemail@example.com')).fetch();
      const [ trail ]:Trail[] = await testDb.get<Trail>('trails').query(Q.where('id', '1')).fetch();

      //get users purchased trails
      let purchasedTrails:User_Purchased_Trail[] = await user.usersPurchasedTrails;
  ...
  describe('buyAddon()',  () => {
    ...
    test('succesfully purchases and adds addon to usersAddons()', async () => {
      ...
      const [ addonToBuy ]: Addon[] = await testDb.get<Addon>('addons').query(Q.where('id', '1')).fetch();
      //get user from localDB
      const [ user  ]:User[] = await testDb.get<User>('users').query( Q.where('email', 'mockemail@example.com')).fetch();
      const oldTrailTokens:number = user.trailTokens;
      //user should have no addons
      let usersAddons: User_Addon[] = await user.usersAddons;
    ...
    test('increases quantity of previously purchased addon', async () => {
      //get user from localDB
      const [ user  ]:User[] = await testDb.get<User>('users').query( Q.where('email', 'mockemail@example.com')).fetch();
      const [alreadyPurchasedAddon]: User_Addon[] = await user.usersAddons;
       const [addonToBuy]: Addon[] = await testDb.get<Addon>('addons').query(Q.where('id', alreadyPurchasedAddon.addonId)).fetch();
      const oldTrailTokens:number = user.trailTokens;
      await user.buyAddon(addonToBuy);
      const [usersAddons]: User_Addon[] = await user.usersAddons;
      expect(usersAddons.quantity).toBe(2);
    ...
    test('throws error if user has doesnt have enough miles', async () => {
      ...
      const [addonToBuy]: Addon[] = await testDb.get<Addon>('addons').query(Q.where('id', '2')).fetch();
      const [ user ]:User[] = await testDb.get<User>('users').query( Q.where('email', 'mockemail@example.com')).fetch();
  ...
  describe('finalizeSessionWithRewardsWriter()', () => {
  ...
  describe('Redeem Park Pass', () => {
    ...
    it('successfully applies reward tokens and adds park pass', async () => {
      ...
      const [ user  ]:User[] = await testDb.get<User>('users').query().fetch();
    ...
    it('successfully applies reward tokens and updates park level correctly when user prestige level is > 0', async () => {
      ...
      const [ user  ]:User[] = await testDb.get<User>('users').query().fetch();
      const previousTokens: number =  await user.trailTokens;
      const [ previousParkPass ]: User_Park[] = await user.usersParks;
