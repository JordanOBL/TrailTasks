...
const DEFAULT_BACKPACK = [{addon: null, minimumTotalMiles:0.0}, {addon: null, minimumTotalMiles:75.0}, {addon: null, minimumTotalMiles:175.0}, {addon: null, minimumTotalMiles:375.0}]
const NewSessionOptions = () => {
   ...
   const categoriesRef = useRef<Session_Category[] | null>(null)
   const currentTrailRef = useRef<Trail | null>(null)
   const currentParkRef = useRef<Park | null>(null)
   ...
   useEffect(() => {
    let alive = true;
     (async () => {
       ...
       try {
         const [sc, [ct]] = await Promise.all([db.get<Session_Category>("session_categories").query().fetch(),
         ...
         if(alive){
            currentTrailRef.current = ct
            currentParkRef.current = park
            categoriesRef.current = sc;
            setLoading(false)
            
         }
       } catch (e) {
         handleError(e, "Error getting  session categories");
       }
     })();
      ...
      <SafeAreaView style={styles.container} testID="new-session-options"> 
        <View style={styles.header}>
          ...
          <Text testID="session-category-display" style={styles.sessionCategory}>
            {categoriesRef.current?.filter(item => item.id === sessionCfg.sessionCategory[0])[0]
        ...
        <View style={styles.middleContainer}>
          <Text style={styles.trailName}>
            {currentTrailRef.current ? currentTrailRef.current.trailName : ""}
          </Text>
          <Text style={styles.parkName}>{currentParkRef.current?.parkName} National Park</Text>
          ...
          <Pressable
            ...
            onPress={async () => {
              try {
                ...
                const sEngine = await sessionEngineMgr.createSessionEngine(sessionCfg, createdSession)
                if (!sEngine) {
        ...
        <SettingsModal
          ...
          sessionCategories={categoriesRef.current}
        />
        {/* Backpack Modal */}
        {/* <BackpackModal
